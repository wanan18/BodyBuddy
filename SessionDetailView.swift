...
struct SessionDetailView: View {
    ...
    private var groupedByExercise: [ExerciseGroup] {
        ...
        .sorted { lhs, rhs in
            // Use the first set's sortIndex (they are already sorted by sortIndex)
            let lIndex = lhs.sets.first?.sortIndex ?? 0
            let rIndex = rhs.sets.first?.sortIndex ?? 0
...
struct AddExerciseToSessionSheet: View {
    ...
    private func save() {
        ...
        // Determine next global sort index for this session
