...
struct TrainView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottomTrailing) {
                ...
                .padding(.trailing, 24)
                .padding(.bottom, 110)   // sits above floating tab bar
            }
            .navigationBarHidden(true)
        }
        .sheet(isPresented: $showingNewSession) {
            NewSessionView()   // make sure this exists or see placeholder below
    ...
    private var cardsList: some View {
        Group {
            if sessions.isEmpty {
                Text("No sessions yet. Use the + button to record your workouts.")
                    .font(.subheadline)
                ...
                VStack(spacing: 12) {
                    ForEach(sessions) { session in
                        NavigationLink {
                            SessionDetailView(session: session) // make sure this exists or use placeholder
                        } label: {
